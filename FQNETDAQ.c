        ...
        for ( it = 0; it < TIMESTAMP_COUNT; ++it ) {
            for ( ch = 0; ch < TDC_QUTAG_CHANNELS; ch++ ) {
                ...
                if(current_delta_time > event_coinc_window  && current_delta_ch < 5 ) {
                    new_event = 1;
                    //time situation means new event.
                }
                else {
                    new_event = 0;
                }
                ...
                if(new_event) {
                    ...
                    //printf("%d %d\n", n_ch, good_event);
                    if( n_ch <=1 ) good_event = 0; //if there is one or less good timestamps in evt_time, its not a good coincidence event.

                    if( good_event ) {
                        NEventsWritten = NEventsWritten + 1;
                        //printf("==new event==\n");
                        //fprintf(data,"\n"); //this seperates events
                        for( ii = 0; ii < 4; ii++) {
                          if( ii < 3 ) fprintf(data,"%"LLDFORMAT" ", evt_time[ii]);
                          else fprintf(data,"%"LLDFORMAT"\n", evt_time[ii]);  //the code is not using this.
                          //!!!Commented out below so empty channels are printed as -666 in datafile (preserved datafile formatting)
                            //if( evt_time[ii] != -666 ) {
                                //printf("%d:%"LLDFORMAT";", ii+1, evt_time[ii]);
                                //fprintf(data,"%"LLDFORMAT" ", evt_time[ii]);
                                //fprintf(data,"%d\n", ii + 1); //include the channel number
                    ...
                    good_event = 1; //event is 1st member of new event. Next run through will add to this event.
                    for( ii = 0; ii < 4; ii++) evt_time[ii] = -666; //configure for this set
                    if( evt_time[channels[index]-1] == -666 ) evt_time[channels[index]-1] = timestamps[index]; //put in pirst of that set
                }
                else {
...
int main( int argc, char ** argv )
{
    ...
    int collect_rounds = 2;//iterations-> total aquisition time is collect_time*collect_rounds;
